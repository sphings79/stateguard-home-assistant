...
- **Creating a second watch overwrote the first.** The panel sends an empty
  id for a new object, and the server used `setdefault`, which only fills a
  key that is *missing*. So every new watch kept the empty id, and the next
  one found that same entry and replaced it. Severities and channels had the
  same flaw.

  Anything already saved without an id is given one when the configuration is
  loaded, and a warning is written to the log. A watch that was overwritten
  before this fix cannot be recovered and has to be created again.
...
[Unreleased]: https://github.com/sphings79/stateguard-home-assistant/compare/v0.2.1...HEAD
[0.2.1]: https://github.com/sphings79/stateguard-home-assistant/compare/v0.2.0...v0.2.1
